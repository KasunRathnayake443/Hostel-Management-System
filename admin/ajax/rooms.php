...
if(isset($_POST['add_room']))
{
    ...
    $q1 = "INSERT INTO `rooms`(`name`, `area`, `fees`, `quantity`, `students`, `description`) VALUES (?,?,?,?,?,?)";
    $values = [$frm_data['name'],$frm_data['area'],$frm_data['fees'],$frm_data['quantity'],$frm_data['students'],$frm_data['desc']];
...
if(isset($_POST['edit_room'])){
    ...
    $q1 = "UPDATE `rooms` SET `name`=?,`area`=?,`fees`=?,`quantity`=?,`students`=?,`description`=? WHERE `id`=?";
    $values = [$frm_data['name'],$frm_data['area'],$frm_data['fees'],$frm_data['quantity'],$frm_data['students'],$frm_data['desc'],$frm_data['room_id']];
...
if(isset($_POST['add_image'])){
  ...
  $img_r = uploadImage($_FILES['image'],IMAGES_FOLDER);
...
if(isset($_POST['get_room_images'])){
  ...
  $path = ROOMS_FOLDER;
   ...
   while($row = mysqli_fetch_assoc($res)){
    if($row['thumb']==1){
      $thumb_btn = "<i class='bi bi-check2 text-align bg-success px-2 py-1 rounded fs-5'></i>";
    }
...
if(isset($_POST['remove_room'])){
  ...
  $res5 = update("UPDATE  `room_facilities` SET `removed`=? WHERE  `id`=?",[$frm_data['room_id']],'ii');
